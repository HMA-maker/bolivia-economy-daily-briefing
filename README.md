...
graph TD
    ...
    K -- Formulario de Suscripción --> L[Google Apps Script Webhook]
    L -- Inserta Lead --> M[(Google Sheet: Suscriptores)]
    B -- Lee Lista Activa CSV --> M
    B -- Envío Masivo Personalizado --> I[Resend API: Suscriptores Activos]
...
| **Landing Page Diaria** | HTML5 / CSS Vanilla / Schema.org | Publicada en `docs/` para **GitHub Pages** (`https://informe.consultoramaldonado.com`). Incluye análisis exhaustivo de deuda, bonos soberanos, selector histórico y formulario de suscripción directa a Google Sheets. |
| **Base de Datos de Suscriptores** | Google Sheets + Apps Script | Captura en tiempo real altas de lectores ejecutivos y solicitudes de asesoría corporativa sin costo ni servidores. |
| **Correo Gancho (Email)** | HTML Inline ultra-ligero | Enviado vía **Resend** a cada suscriptor activo con saludo personalizado ("Estimado/a Juan:"). Incluye la infografía, Matriz TL;DR de alertas (🔴/🟡/🟢), 3 bloques snapshot y botón CTA para visitar la web. |
...
# Base de Datos de Contactos y Suscriptores (Google Sheets)
GOOGLE_SCRIPT_WEBHOOK_URL=https://script.google.com/macros/s/AKfycbw.../exec
SUBSCRIBERS_SHEET_CSV_URL=https://docs.google.com/spreadsheets/d/TU_SHEET_ID/export?format=csv
